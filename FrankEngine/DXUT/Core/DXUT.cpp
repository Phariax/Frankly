...
int g_minWindowSizeX = 128;
int g_minWindowSizeY = 128;

// hack, fix min window sizes based on adjusted rect
static int adjustedMinWindowSizeX = 0;
static int adjustedMinWindowSizeY = 0;
                                 ...
                                 HICON hIcon, HMENU hMenu, int x, int y )
{
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
       ...
       // try to use second icon as the small icon for the corner of the window's title bar
       HICON hIconSmall = ExtractIcon( hInstance, szExePath, 1 );
	   if (!hIconSmall)
		   hIconSmall = hIcon;
		...
		//DWORD windowStyle = WS_DLGFRAME;			// a simple window without resize and x in corner
		//DWORD windowStyle = WS_POPUP;				// a window that has no frame at all and can't be moved
		DWORD windowStyle = WS_OVERLAPPEDWINDOW;	// the normal style
        HWND hWnd = CreateWindow( L"Direct3DWindowClass", strWindowTitle, windowStyle,
...
LRESULT CALLBACK DXUTStaticWndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_GETMINMAXINFO:
            ((MINMAXINFO*)lParam)->ptMinTrackSize.x = adjustedMinWindowSizeX;
            ((MINMAXINFO*)lParam)->ptMinTrackSize.y = adjustedMinWindowSizeY;
            break;
...
//--------------------------------------------------------------------------------------
// set a bool so it can reset the device at the proper time so we can toggle vsync
static bool s_resetDevice = false;
void WINAPI DXUTResetDevice()
{
	s_resetDevice = true;
}
...
HRESULT WINAPI DXUTMainLoop( HACCEL hAccel )
{
    ...
    while( WM_QUIT != msg.message  )
    {
        ...
        else
        {
			...
			if (s_resetDevice)
			{
				// this is just a simple way to reset the device so we can toggle vsync
...
HRESULT WINAPI DXUTCreateDevice( bool bWindowed, int nSuggestedWidth, int nSuggestedHeight )
{
	...
	{
		// hack: adjust min window size
		RECT rcMin;
		SetRect( &rcMin, 0, 0, g_minWindowSizeX, g_minWindowSizeY );        
		AdjustWindowRect( &rcMin, WS_OVERLAPPEDWINDOW, (GetMenu(DXUTGetHWNDDeviceWindowed())) ? true : false );
		adjustedMinWindowSizeX = rcMin.right - rcMin.left;
		adjustedMinWindowSizeY = rcMin.bottom - rcMin.top;
	}
...
bool DXUTSkipNextRenderHack = false;
...
void DXUTRender3DEnvironment9()
{
	...
	if (DXUTSkipNextRenderHack)
		 DXUTSkipNextRenderHack = false;
	else if( !GetDXUTState().GetRenderingPaused())
    {
...
UINT WINAPI DXUTGetRefreshRate()                           
{
	D3DDISPLAYMODE adapterDesktopDisplayMode;
    IDirect3D9* pD3D = DXUTGetD3D9Object();
	if (!pD3D)
    ...
    DXUTDeviceSettings* pDeviceSettings = GetDXUTState().GetCurrentDeviceSettings();
	assert(DXUTIsD3D9(pDeviceSettings)); // this function is not yet implemented for d3d10
	if (!pDeviceSettings)
		return 0;

    pD3D->GetAdapterDisplayMode( pDeviceSettings->d3d9.AdapterOrdinal, &adapterDesktopDisplayMode );
	return adapterDesktopDisplayMode.RefreshRate;
};
...
void WINAPI DXUTSetWindowBackBufferSizeAtModeChange( int width, int height )
{
	GetDXUTState().SetWindowBackBufferWidthAtModeChange(width);
	GetDXUTState().SetWindowBackBufferHeightAtModeChange(height);
}
