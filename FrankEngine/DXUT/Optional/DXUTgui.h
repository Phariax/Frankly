...
public:
	...
	void BeginSprite();
	void EndSprite();
	int GetTextLength( const WCHAR* strMsg, UINT font );
    HRESULT DrawText( LPCWSTR strText, UINT font, const RECT& prcDest, D3DXCOLOR color, DWORD dwTextFormat = 0, int shadowOffset = 0);
    HRESULT DrawTextImmediate( LPCWSTR strText, UINT font, const RECT& prcDest, D3DXCOLOR color, DWORD dwTextFormat = 0, int shadowOffset = 0);
    ...
    static CDXUTControl* GetFocus() { return s_pControlFocus; } 
    CDXUTControl* GetMouseOver() { return m_pControlMouseOver; } 
...
public:
    ...
    virtual void SetFont( UINT iFont );
...
protected:
    ...
    WCHAR m_strText[maxLength];      // Window text  
};
