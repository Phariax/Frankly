...
//--------------------------------------------------------------------------------------
HRESULT CDXUTTextHelper::DrawTextLine( const WCHAR* strMsg, bool advanceUp, bool dropShadow )
{
	FrankProfilerEntryDefine(L"TextHelper::DrawTextLine()", Color::White(), 999);
    ...
    if( m_pFont9 )
	{
		if (dropShadow)
		{
			...
			hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, D3DXCOLOR(0,0,0,m_clr.a) );
		}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTTextHelper::DrawTextLine( int x, int y, const WCHAR* strMsg, const D3DXCOLOR& color, bool centerX, bool centerY)
{
	if( NULL == m_pFont9 && NULL == m_pFont10 ) 
        return DXUT_ERR_MSGBOX( L"DrawTextLine", E_INVALIDARG );
	...
	{
		RECT rcSize = rc;
		m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rcSize, DT_NOCLIP|DT_CALCRECT, m_clr );
		if (centerX)
			rc.left -= (rcSize.right - rc.left)/2;
		if (centerY)
			rc.top -= (rcSize.bottom - rc.top)/2;
	}
	
	hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, color );
    if( FAILED(hr) )
        return DXTRACE_ERR_MSGBOX( L"DrawText", hr );
...
HRESULT DXUTSnapD3D9Screenshot( LPCTSTR szFileName )
{
    ...
    hr = D3DXSaveSurfaceToFile( szFileName, D3DXIFF_PNG, pBackBuffer, NULL, NULL );
