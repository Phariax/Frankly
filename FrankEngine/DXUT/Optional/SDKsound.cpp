...
HRESULT CSoundManager::SetSpeakerConfig(DWORD speakerConfig)
{
	return m_pDS->SetSpeakerConfig(speakerConfig);
}
                ...
                DWORD dwNumBuffers, CWaveFile* pWaveFile, DWORD dwCreationFlags )
{
    ...
    m_apDSBuffer = new BufferWrapper[dwNumBuffers];
    if( NULL != m_apDSBuffer )
    {
        for( i=0; i<dwNumBuffers; i++ )
            m_apDSBuffer[i].buffer = apDSBuffer[i];
        ...
        FillBufferWithSound( m_apDSBuffer[0].buffer, FALSE );
    }
}
...
CSound::~CSound()
{
    ...
    {
        SAFE_RELEASE( m_apDSBuffer[i].buffer );
    }
...
// update frequency to conform to time scale
void CSound::UpdateTimeScale( float scale )
{
    for (DWORD i=0; i<m_dwNumBuffers; i++ )
    {
        if (!m_apDSBuffer[i].buffer)
			continue;

		LPDIRECTSOUNDBUFFER pDSB = m_apDSBuffer[i].buffer;
		...
		pDSB->GetFormat(&waveFormat, sizeof(WAVEFORMATEX), NULL);
		const DWORD dwBaseFrequency = waveFormat.nSamplesPerSec;

		// scale the frequency
		DWORD f = DWORD(dwBaseFrequency * (scale * m_apDSBuffer[i].frequencyScale));

		// hack, dont let it equal 0 caus that will set it to default
		if (f == 0)
			f = DSBFREQUENCY_MIN;
        pDSB->SetFrequency( f );
    }
...
CSound::BufferWrapper* CSound::GetFreeBuffer()
{
    ...
    for( i=0; i<m_dwNumBuffers; i++ )
    {
        ...
        {
            DWORD dwStatus = 0;
            m_apDSBuffer[i].buffer->GetStatus( &dwStatus );
            if ( ( dwStatus & DSBSTATUS_PLAYING ) == 0 )
                break;
        }
    }
    ...
    if( i != m_dwNumBuffers )
        ...
        return NULL;//m_apDSBuffer[ rand() % m_dwNumBuffers ];
}
...
// Desc:
//-----------------------------------------------------------------------------
CSound::BufferWrapper* CSound::GetBuffer( DWORD dwIndex )
{
    if( m_apDSBuffer == NULL )
        return NULL;
...
HRESULT CSound::Get3DBufferInterface( DWORD dwIndex, LPDIRECTSOUND3DBUFFER* ppDS3DBuffer )
{
    ...
    *ppDS3DBuffer = NULL;
    ...
    return m_apDSBuffer[dwIndex].buffer->QueryInterface( IID_IDirectSound3DBuffer,
                                                  (VOID**)ppDS3DBuffer );
}
...
//-----------------------------------------------------------------------------
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, float volumePercent, float frequencyScale, LONG lPan, BufferWrapper** buffer )
{
    HRESULT hr;
    BOOL    bRestored;
   ...
   BufferWrapper* pDSB = GetFreeBuffer();

    if( pDSB == NULL )
        return DXUT_ERR( L"GetFreeBuffer", E_FAIL );

	pDSB->frequencyScale = frequencyScale;
    ...
    {
		const LONG lVolume = (LONG)(DSBVOLUME_MIN + volumePercent * (DSBVOLUME_MAX - DSBVOLUME_MIN - 1));
        pDSB->buffer->SetVolume( lVolume );
    }
    ...
    if( m_dwCreationFlags & DSBCAPS_CTRLFREQUENCY )
    {
		// get the base frequency
		WAVEFORMATEX waveFormat;
		pDSB->buffer->GetFormat(&waveFormat, sizeof(WAVEFORMATEX), NULL);
		const DWORD dwBaseFrequency = waveFormat.nSamplesPerSec;

        pDSB->buffer->SetFrequency( DWORD(dwBaseFrequency * frequencyScale) );
    }

    if( m_dwCreationFlags & DSBCAPS_CTRLPAN )
    {
        pDSB->buffer->SetPan( lPan );
    }

    return pDSB->buffer->Play( 0, dwPriority, dwFlags );
}
...
//       in the dwFlags to loop the sound
//-----------------------------------------------------------------------------
HRESULT CSound::Play3D( LPDS3DBUFFER p3DBuffer, DWORD dwPriority, DWORD dwFlags, float volumePercent, float frequencyScale, BufferWrapper** buffer )
{
    HRESULT hr;
    BOOL    bRestored;
    ...
    hr = pDSB->buffer->QueryInterface( IID_IDirectSound3DBuffer, (VOID**) &pDS3DBuffer );
    if( SUCCEEDED( hr ) )
    {
        hr = pDS3DBuffer->SetAllParameters( p3DBuffer, DS3D_IMMEDIATE );
        if( SUCCEEDED( hr ) )
            hr = pDSB->buffer->Play( 0, dwPriority, dwFlags );
...
HRESULT CSound::Reset()
{
    ...
    for( DWORD i=0; i<m_dwNumBuffers; i++ )
        hr |= m_apDSBuffer[i].buffer->SetCurrentPosition( 0 );
...
HRESULT CStreamingSound::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
    ...
    // Restore the buffer if it was lost
    BOOL bRestored;
    if( FAILED( hr = RestoreBuffer( m_apDSBuffer[0].buffer, &bRestored ) ) )
        return DXUT_ERR( L"RestoreBuffer", hr );

    if( bRestored )
    {
        // The buffer was restored, so we need to fill it with new data
        if( FAILED( hr = FillBufferWithSound( m_apDSBuffer[0].buffer, FALSE ) ) )
            return DXUT_ERR( L"FillBufferWithSound", hr );
        return S_OK;
    }

    // Lock the DirectSound buffer
    if( FAILED( hr = m_apDSBuffer[0].buffer->Lock( m_dwNextWriteOffset, m_dwNotifySize,
                                            &pDSLockedBuffer, &dwDSLockedBufferSize,
    ...
    m_apDSBuffer[0].buffer->Unlock( pDSLockedBuffer, dwDSLockedBufferSize, NULL, 0 );
    ...
    if( FAILED( hr = m_apDSBuffer[0].buffer->GetCurrentPosition( &dwCurrentPlayPos, NULL ) ) )
...
HRESULT CStreamingSound::Reset()
{
    ...
    if( m_apDSBuffer[0].buffer == NULL || m_pWaveFile == NULL )
        return CO_E_NOTINITIALIZED;
    ...
    return m_apDSBuffer[0].buffer->SetCurrentPosition( 0L );
}
