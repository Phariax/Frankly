...
public:
    ...
    BufferWrapper* GetBuffer( DWORD dwIndex );

    HRESULT Play( DWORD dwPriority = 0, DWORD dwFlags = 0, float volumePercent = 1.0f, float frequencyScale = 1.0f, LONG lPan = 0, BufferWrapper** buffer = 0 );
    HRESULT Play3D( LPDS3DBUFFER p3DBuffer, DWORD dwPriority = 0, DWORD dwFlags = 0, float volumePercent = 1.0f, float frequencyScale = 0, BufferWrapper** buffer = 0 );
    HRESULT Stop();
    HRESULT Reset();
    BOOL    IsSoundPlaying();
