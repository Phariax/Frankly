...
//--------------------------------------------------------------------------------------
void CDXUTDialog::BeginSprite()	
{ 
	m_pManager->m_pSprite->Begin( D3DXSPRITE_DONOTSAVESTATE ); 
}

//--------------------------------------------------------------------------------------
void CDXUTDialog::EndSprite()	
{ 
	m_pManager->m_pSprite->End(); 

	// hack: put lighting back on
	m_pManager->GetD3D9Device()->SetRenderState(D3DRS_LIGHTING, TRUE);
}

//--------------------------------------------------------------------------------------
int CDXUTDialog::GetTextLength( const WCHAR* strMsg, UINT font )
{
    RECT rcSize;
    DXUTFontNode* pFontNode = GetFont( font );
	pFontNode->pFont9->DrawText( m_pManager->m_pSprite, strMsg, -1, &rcSize, DT_NOCLIP|DT_CALCRECT, D3DCOLOR_ARGB(255,0,0,0) );
    return rcSize.right - rcSize.left;
}
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawTextImmediate( LPCWSTR strText, UINT font, const RECT& prcDest, const D3DXCOLOR color, DWORD dwTextFormat, int shadowOffset)
{
	BeginSprite();
	HRESULT hr = DrawText( strText, font, prcDest, color, dwTextFormat, shadowOffset);
	EndSprite();
	return hr;
}

//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawText( LPCWSTR strText, UINT font, const RECT& prcDest, const D3DXCOLOR color, DWORD dwTextFormat, int shadowOffset)
{
	HRESULT hr = S_OK;
    ...
    if( shadowOffset > 0 )
    {
        ...
        OffsetRect( &rcShadow, shadowOffset, shadowOffset );
        hr = pFontNode->pFont9->DrawText( m_pManager->m_pSprite, strText, nCount, &rcShadow, dwTextFormat, D3DCOLOR_ARGB(DWORD(color.a * 255), 0, 0, 0) );
        if( FAILED(hr) )
            return hr;
    }

    hr = pFontNode->pFont9->DrawText( m_pManager->m_pSprite, strText, nCount, &rcScreen, dwTextFormat, color );
...
HRESULT CDXUTDialogResourceManager::CreateTexture9( UINT iTexture )
{
    ...
    if( !pTextureNode->bFileSource )
    {
        if( pTextureNode->nResourceID == 0xFFFF && pTextureNode->hResourceModule == (HMODULE) 0xFFFF )
        {
			...
			// hack: save out ui image
			//LPDIRECT3DSURFACE9 surface = NULL;
			//pTextureNode->pTexture9->GetSurfaceLevel(0, &surface);
			//D3DXSaveSurfaceToFile( L"ui.png", D3DXIFF_PNG, surface, NULL, NULL );
...
//--------------------------------------------------------------------------------------
bool CDXUTControl::HasFocus()
{
	return (CDXUTDialog::GetFocus() == this);
}
...
void CDXUTControl::SetTextColor( D3DCOLOR Color )
{
	...
	// HACK: set the color directly so the change is happens immediatly
...
HRESULT CDXUTStatic::SetText( LPCWSTR strText )
{
    ...
    StringCchCopy( m_strText, maxLength, strText); 
    return S_OK;
}
...
//--------------------------------------------------------------------------------------
void CDXUTButton::SetTextColor( D3DCOLOR Color )
{
	// FF: I fixed this to work with buttons
...
void CDXUTEditBox::SetText( LPCWSTR wszText, bool bSelected )
{
    ...
    m_nFirstVisible = 0;
    // Move the caret to the front of the text
    PlaceCaret( 0 );
...
void CDXUTEditBox::PasteFromClipboard()
{
    ...
    if( OpenClipboard( NULL ) )
    {
        ...
        if( handle )
        {
            ...
            if( pwszText )
            {
				const int maxPasteLength = 1000;
				WCHAR buffer[maxPasteLength];

				// hack: get rid of new lines and carrige returns, use them as delimiters
				for(int i = 0; i < maxPasteLength; ++i)
				{
					if (pwszText[i] == 10 || pwszText[i] == 13 || pwszText[i] == 0)
					{
						buffer[i] = '\0';
						break;
					}
                ...
                if( m_Buffer.InsertString( m_nCaret, buffer ) )
                    PlaceCaret( m_nCaret + lstrlenW( buffer ) );
                m_nSelStart = m_nCaret;
            }
        }
        CloseClipboard();
...
void CDXUTEditBox::ParseFloatArray( float *pNumbers, int nCount )
{
    ...
    while( nWritten < nCount && *pToken != L'\0' )
    {
        ...
        int nTokenLen = __min( sizeof(wszToken) / sizeof(wszToken[0]) - 1, int(pEnd - pToken + 1) );
...
void CDXUTEditBox::SetTextFloatArray( const float *pNumbers, int nCount )
{
    ...
    {
        StringCchPrintf( wszTmp, 64, L"%.9f ", pNumbers[i] );
