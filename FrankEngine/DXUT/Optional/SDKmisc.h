...
protected:
	...
	// this was conflicting with the definition in dxut, I'm not sure why it was set up like this
...
public:
    ...
    POINT GetInsertionPos() { return m_pt; }

    void Begin();
    HRESULT DrawFormattedTextLine( const WCHAR* strMsg, ... );
    HRESULT DrawTextLine( const WCHAR* strMsg, bool advanceUp = false, bool dropShadow = false );
    ...
    HRESULT DrawTextLine( int x, int y, const WCHAR* strMsg, const D3DXCOLOR& color, bool centerX = false, bool centerY = false );
    void End();
