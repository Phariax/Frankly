...
static Vector2 halfPixel(-1, 1);
bool DeferredRender::SpecularRenderBlock::active = 0;
bool DeferredRender::TransparentRenderBlock::active = 0;
float DeferredRender::DiffuseNormalAlphaRenderBlock::alphaScale = 1;
bool DeferredRender::DiffuseNormalAlphaRenderBlock::active = 0;
...
static void ConsoleCommandCallback_lightShowTexture(const wstring& text)
{
	swscanf_s(text.c_str(), L"%d", &DeferredRender::showTexture);
	
	switch(DeferredRender::showTexture)
	{
		case 0: GetDebugConsole().AddLine(L"Show Texture: 0 None"); break;
		case 1: GetDebugConsole().AddLine(L"Show Texture: 1 Final"); break;
		case 2: GetDebugConsole().AddLine(L"Show Texture: 2 Normals"); break;
		case 3: GetDebugConsole().AddLine(L"Show Texture: 3 Specular"); break;
		case 4: GetDebugConsole().AddLine(L"Show Texture: 4 Emissive"); break;
		case 5: GetDebugConsole().AddLine(L"Show Texture: 5 ShadowMap"); break;
		case 6: GetDebugConsole().AddLine(L"Show Texture: 6 BackgroundShadowMap"); break;
		case 7: GetDebugConsole().AddLine(L"Show Texture: 7 Diffuse"); break;
		case 8: GetDebugConsole().AddLine(L"Show Texture: 8 Vision"); break;
	}
}
ConsoleCommand(ConsoleCommandCallback_lightShowTexture, lightShowTexture);

bool DeferredRender::showFinalAccumulator = false;
ConsoleCommand(DeferredRender::showFinalAccumulator, showFinalAccumulator);

// save shadows to disk for debugging
bool shadowSaveTextures = false;
ConsoleCommand(shadowSaveTextures, shadowSaveTextures);

// prevent flicker between stationary lights and objects by rounding to nearest pixel
bool lightPixelRoundingEnable = true;
ConsoleCommand(lightPixelRoundingEnable, lightPixelRoundingEnable);
...
// globaly scales alpha of all halo colors
float DeferredRender::haloAlpha = 0.4f;
ConsoleCommand(DeferredRender::haloAlpha, lightHaloAlpha);
...
float DeferredRender::directionalLightCastScale = 2.0f;
...
float DeferredRender::directionalLightRedrawAlpha = 0.8f;
ConsoleCommand(DeferredRender::directionalLightRedrawAlpha, directionalLightRedrawAlpha);
...
float DeferredRender::specularAmmount = 1;
ConsoleCommand(DeferredRender::specularAmmount, specularAmmount);
...
void DeferredRender::UpdateSimpleLights(list<Light*> simpleLights)
{
	...
	g_cameraBase->PrepareForRender(xfFinal, finalTextureCameraScale*g_cameraBase->GetZoomInterpoalted());
	...
	for (list<Light*>::iterator it = simpleLights.begin(); it != simpleLights.end(); ++it)
	{
		Light& light = **it;
...
void DeferredRender::UpdateSimpleLight(Light& light, const XForm2& xfFinal, const Vector2& cameraSize)
{
	...
	const XForm2 xf = light.GetInterpolatedXForm();
	...
	{
		g_render->RenderQuad(XForm2(halfPixelOffset * light.radius)*xf, Vector2(light.radius), finalColor, g_render->GetTexture(light.gelTexture? light.gelTexture : GameTexture_LightMask, false));
	}
	else
	{
		{	
		...
		pd3dDevice->SetTexture(2, g_render->GetTexture(light.gelTexture? light.gelTexture : GameTexture_LightMask, false));
...
void DeferredRender::UpdateDynamicLight(Light& light)
{
	...
	const XForm2 xfInterpolated = light.GetInterpolatedXForm();
	...
	pd3dDevice->BeginScene();
	{
		...
		else
		{
			...
			shadowLightConstantTable->SetFloat(pd3dDevice, "specularAmmount", specularAmmount);
...
void DeferredRender::RenderLightTexture(Light& light)
{
	...
	const GameTextureID& gelTexture = light.gelTexture;
	...
	pd3dDevice->SetRenderTarget(0, renderSurface);
	pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, Color::White(), 1.0f, 0);
	pd3dDevice->BeginScene();
	{
		...
		if (overbrightRadius > 0)
		{
			...
			pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_ONE);
			pd3dDevice->SetRenderState( D3DRS_BLENDOP, D3DBLENDOP_ADD );
			g_render->RenderQuad(xf, Vector2(overbrightRadius), Color::White(), GameTexture_Circle, false);
		}
		...
		if (castShadows && shadowEnable)
		{
			...
			*/
			...
			while (1)
			{
				...
				{
					brightness = Percent((float)passCount, 5.0f, 2.0f);
				...
				// soften
				g_render->RenderQuad(halfPixelOffset, Vector2(1), Color::Grey(1, brightness), GameTexture_LightMask, false);
		...
		}
		{
			...
			GameTextureID ti = gelTexture? gelTexture : GameTexture_LightMask;
...
void DeferredRender::RenderCone(Light& light, float lightAngle)
{
	...
	const float insideBlurStep = coneFadeAngle/32;
...
void DeferredRender::InitDeviceObjects()
{
	...
	{
		if (!g_render->LoadPixelShader(L"data/shaders/blurShader.psh", blurShader, blurConstantTable))
	...
	{
		if (!g_render->LoadPixelShader(L"data/shaders/visionShader.psh", visionShader, visionConstantTable))
	...
	if (!g_render->LoadPixelShader(L"data/shaders/directionalLightShader.psh", directionalLightShader, directionalLightConstantTable))
	...
	if (normalMappingEnable)
	{
		...
		if (!g_render->LoadPixelShader(L"data/shaders/deferredLightShader.psh", deferredLightShader, shadowLightConstantTable))
...
{
	{
		...
		if (g_gameControlBase->IsEditMode() || !lightEnable)
			return;

		if (!g_render->GetTexture(GameTexture_LightMask))
		{
		...
		if (!g_render->GetTexture(GameTexture_Circle))
		{
			g_debugMessageSystem.AddError( L"Circle texture not found. Lighting disabled!" );
			lightEnable = false;
			return;
		}

		ASSERT(g_render->GetTexture(GameTexture_LightMask));	// you must load a light mask texture to use lighting
		ASSERT(g_render->GetTexture(GameTexture_Circle));	// you must load a circle mask texture to use lighting
	...
	}
	{
		...
		for (GameObjectHashTable::iterator it = objects.begin(); it != objects.end(); ++it)
		{
			...
			if (!obj.IsLight() || obj.IsDestroyed())
				continue;
...
void DeferredRender::SwapTextures(LPDIRECT3DTEXTURE9& texture1, LPDIRECT3DTEXTURE9& texture2)
{
	...
	LPDIRECT3DTEXTURE9 tempTexture = texture1;
	texture1 = texture2;
...
void DeferredRender::GlobalRender()
{
	...
	if (showTexture > 0)
	{
		...
		else if (showTexture == 4)
		{
			...
			const float zoom = g_cameraBase->GetZoomInterpoalted() * oldAspect;
	...
	if (visionEnable && showTexture == 0)
	{
		const XForm2 xfPlayer = g_gameControlBase->GetUserPositionInterpolated();
...
void DeferredRender::Invert(LPDIRECT3DTEXTURE9& texture, const Vector2& invertTextureSize)
{
	...
	pd3dDevice->SetTransform(D3DTS_VIEW, &matrixIdentity);
	{
		...
		pd3dDevice->SetRenderTarget(0, renderSurface);
		pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, Color(1,1,1,1), 1.0f, 0);
		pd3dDevice->BeginScene();
...
void DeferredRender::ApplyBlur(LPDIRECT3DTEXTURE9& texture, const Vector2& blurTextureSize, float brightness, float blurSize, int passCount)
{
	...
	for(int i = 0; i < passCount; ++i)
	{
		...
		(texture2)->GetSurfaceLevel(0, &renderSurface);
		pd3dDevice->SetRenderTarget(0, renderSurface);
		pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, Color(0,0,0,1), 1.0f, 0);
		pd3dDevice->BeginScene();
		{
...
{
	// use max zoom for shadow map normall, but go large if camera scales beyond max
	const float zoomInterp = g_cameraBase->GetZoomInterpoalted();
...
void DeferredRender::RenderNormalMap()
{
	...
	g_cameraBase->PrepareForRender(xf, finalTextureCameraScale*g_cameraBase->GetZoomInterpoalted());
	
	// prepare render to normal map
	LPDIRECT3DSURFACE9 renderSurface = NULL;
	textureNormalMap->GetSurfaceLevel(0, &renderSurface);
 	pd3dDevice->SetRenderTarget(0, renderSurface);
	pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, Vector3(0, 0, 1).GetDWORD(), 1.0f, 0);
	pd3dDevice->BeginScene();
	{
...
void DeferredRender::RenderShadowMap()
{
	...
	// todo: get subtractive blending worning, set up shader
	//pd3dDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
	//pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_ONE);
	//pd3dDevice->SetRenderState( D3DRS_BLENDOP, D3DBLENDOP_REVSUBTRACT);
...
void DeferredRender::RenderEmissivePass()
{
	...
	pd3dDevice->SetRenderTarget(0, renderSurface);
	pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, emissiveBackgroundColor, 1.0f, 0);
	pd3dDevice->BeginScene();
	{
		// render the shadow objects
...
void DeferredRender::RenderDirectionalPass()
{
	...
	pd3dDevice->BeginScene();
	{
		...
		const Vector2 halfPixelOffset = halfPixel / textureSize;
		float offset = directionalLightCastScale;
		int passCount = 0;
		while (1)
		{
			// stretch out the shadow
			...
			g_render->RenderQuad(halfPixelOffset + actualOffset*lightDirection, Vector2(1), Color::Grey(1, directionalLightSoftening), textureSwap);

			if (directionalLightRedrawAlpha > 0)
			{
				// redraw the shadow map every time to keep stauff dark
				{
					Vector2 cameraSize;
					const float shadowMapZoom = GetShadowMapZoom();
					XForm2 xf = GetFinalTransform(Vector2(textureSize), &cameraSize, &shadowMapZoom);
				...
				cameraSize *= shadowMapScale;
				Vector2 offset = cameraSize * halfPixelOffset;
				g_render->RenderQuad(offset + xf.position, cameraSize, Color::White(directionalLightRedrawAlpha), textureShadowMap);
			...
			offset += directionalLightCastScale;
	...
	pd3dDevice->BeginScene();
	{
		...
		else
		{
			{	
				// create a transform to convert texture space to normal map space
				const Vector2 size((shadowMapZoom)/g_cameraBase->GetZoomInterpoalted());
				Vector2 offset = 0.5f*size*(xf.position - xfFinal.position)/cameraSize;
				offset = 0.5f*(Vector2(1) - size) + offset * Vector2(1,-1);

				// correct for half pixel offset in normal map
				offset -= Vector2(shadowMapZoom)/(2*textureSize*g_cameraBase->GetZoomInterpoalted());
			...
			directionalLightConstantTable->SetFloat(pd3dDevice, "specularAmmount", specularAmmount);
...
void DeferredRender::RenderVisionPass()
{
	FrankProfilerEntryDefine(L"DeferredRender::RenderVision()", Color::White(), 10);

	if (!lightEnable || !visionEnable || showTexture > 0 && showTexture != 7)
		return;

	RenderVisionShadowMap();

	Color color = Color::White();
	const XForm2 xfInterpolated = g_gameControlBase->GetUserPositionInterpolated();
	const XForm2 xf(xfInterpolated.position);	// wipe out angle from light calculations
	...
	// prepare render to shadow texture
	LPDIRECT3DTEXTURE9& textureSwap = GetSwapTexture(textureVision);
	LPDIRECT3DSURFACE9 renderSurface = NULL;
	textureVision->GetSurfaceLevel(0, &renderSurface);
	pd3dDevice->SetRenderTarget(0, renderSurface);
	pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, Color(1,1,1,1), 1.0f, 0);
	pd3dDevice->BeginScene();
	{
		...
		
		{
			...
			if (visionOverbrightRadius > 0)
			{
				...
				pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_ONE);
				pd3dDevice->SetRenderState( D3DRS_BLENDOP, D3DBLENDOP_ADD );
				g_render->RenderQuad(XForm2(0.5f*visionOverbrightRadius*cameraSize * halfPixelOffset)*xf, Vector2(visionOverbrightRadius), Color::White(1.0f), GameTexture_Circle, false);
			}
		}
		...
		{
			{
...

				{
					...
					pd3dDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
					pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_ONE);
					ApplyBlur(textureVision, Vector2(visionTextureSize), 1.2f, 2, visionPreBlurPassCount);
				}
				...
				SwapTextures(textureVision, textureSwap);
			...
			while (1)
			{
				...
				g_render->RenderQuad(halfPixelOffset, Vector2(1.0f), Color::Grey(1, passPercent*visionSoftening), GameTexture_Invalid, false);
		...
		//	pd3dDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_ZERO);
		//	pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_SRCCOLOR);
		//	g_render->RenderQuad(halfPixelOffset, Vector2(1), Color::White(), GameTexture_LightMask, false);
		//}
		pd3dDevice->EndScene();
	...
	
	{
		...
		pd3dDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
		pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_ONE);
		ApplyBlur(textureVision, Vector2(visionTextureSize), 1.8f, 2, visionPostBlurPassCount);
	}
