...
Line2 TerrainTile::cachedEdgeLines[TERRAIN_UNIQUE_TILE_COUNT] = {Line2(0.0f)};
Vector2 TerrainTile::cachedPolyVerts[TERRAIN_UNIQUE_TILE_COUNT][TERRAIN_TILE_MAX_VERTS] = {Vector2(0.0f)};
...
bool TerrainTile::MakeSmaller(const Vector2& pos, const Vector2& direction)
{
	...
	const GameSurfaceInfo& tile1Info = GameSurfaceInfo::Get(surface0);
	const GameSurfaceInfo& tile2Info = GameSurfaceInfo::Get(surface1);
	if (tile1Info.HasCollision() && tile2Info.HasCollision() && GetSurfaceHasArea(false) && GetSurfaceHasArea(true))
	{
		SetSurfaceData(pos, 0);
		return true;
	}

	if (tile1Info.HasCollision() && tile2Info.HasCollision())
	{
		// if both surfaces have collision we need to reset the tile with a empty surface
		if (GetSurfaceHasArea(true))
			surface0 = 0;
		else
			surface1 = 0;

		// todo: pick the one with less area or something

		// set both points to be the nearest point
		const BYTE newHalfByte = GetHalfByteFromPosition(pos);
		SetHalfBytes(newHalfByte, newHalfByte);
	}
	else
	{
		if (!tile1Info.HasCollision())
		{
			// flip it
			BYTE surfaceTemp = surface0;
			surface0 = surface1;
			surface1 = surfaceTemp;
			edgeByte = GetInvertedEdgeData();
		}

		// needs one solid surface and one non solid surface
		ASSERT(tile1Info.HasCollision() && !tile2Info.HasCollision() || tile2Info.HasCollision() && !tile1Info.HasCollision());
	}
	...
	if (!GetSurfaceHasArea(true))
	{
		...
		const BYTE newHalfByte = GetHalfByteFromPosition(pos, false);
	...
	const float distanceA = (pos - GetPosA()).MagnitudeSquared();
	const float distanceB = (pos - GetPosB()).MagnitudeSquared();
...
void TerrainTile::BuildVertList(BYTE edgeByte, Vector2* vertices, int& vertexCount)
{
	...
	if (byteA >= 4 && byteA <= 7 && byteB >= 4 && byteB <= 7 && byteA >= byteB)
		byteA = byteB = 0;
	
	if (byteA >= 8 && byteA <= 11 && byteB >= 8 && byteB <= 11 && byteA >= byteB)
		byteA = byteB = 0;
	
	if (byteA >= 12 && byteA <= 15 && byteB >= 12 && byteB <= 15 && byteA >= byteB)
		byteA = byteB = 0;
