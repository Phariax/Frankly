...
struct TerrainTile
{
	TerrainTile(BYTE _edgeByte = 0, BYTE _surface0 = 0, BYTE _surface1 = 0, BYTE _tileSet = 0) :
		edgeByte(_edgeByte),
		surface0(_surface0),
		surface1(_surface1),
		tileSet(_tileSet)
	...
	void MakeClear() { edgeByte = surface0 = surface1 = tileSet = 0; }
	...
	void GetVertList(const Vector2*& vertices, int& vertexCount) const
	{ GetVertList(vertices, vertexCount, edgeByte); }
	...
	TerrainTile Invert() const { return TerrainTile( GetInvertedEdgeData(), surface1, surface0, tileSet ); }

	// does the surface have actual area or is it one dimensional?
	bool GetSurfaceHasArea(int side) const
	{ return side? (!IsFull() && cachedPolyVertCount[GetInvertedEdgeData()] > 2) : (cachedPolyVertCount[GetEdgeData()] > 2); }
...
public: // get and set functions for data
	...
	// tile set functions
	void SetTileSet(BYTE t) { tileSet = t; }
	BYTE GetTileSet() const { return tileSet; }
...
private:
	...
	BYTE tileSet;	// tile set used by this tile
...
inline int TerrainTile::GetSurfaceSide(const Vector2& offset) const
{
	...
	return Vector2::IsClockwise(line.p1, line.p2, offset);
}
