...
GameSurfaceInfo* GameSurfaceInfo::gameSurfaceInfoArray = NULL;

bool GameSurfaceInfo::NeedsEdgeCollision(const TerrainTile& tile)
{
	// this only applies to edge loop type terrain physics
	const GameSurfaceInfo& info1 = Get(tile.GetSurfaceData(false));
	const GameSurfaceInfo& info2 = Get(tile.GetSurfaceData(true));
...
void GameSurfaceInfo::Init(GameSurfaceInfo* _gameSurfaceInfoArray)
{
	gameSurfaceInfoArray = _gameSurfaceInfoArray;
