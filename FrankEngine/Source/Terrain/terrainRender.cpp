...
#define D3DFVF_TERRAIN_VERTEX (D3DFVF_XYZ|D3DFVF_TEX1|D3DFVF_TEXCOORDSIZE2(0))
...
static bool terrainCacheEnable = true;
ConsoleCommand(terrainCacheEnable, terrainCacheEnable)
...
{
	if (!enableRender || !terrainCacheEnable || !g_gameControlBase->IsGameplayMode())
	{
	...
	for (int k = 0; k < tileBufferCount; ++k)
	{
		...
		const IntVector2 checkPatchOffset = g_terrain->GetPatchOffset(centerPatchPos);
		if (checkPatchOffset.x < i1 || checkPatchOffset.x > i2 || checkPatchOffset.y < j1 || checkPatchOffset.y > j2)
		{
			for (int l = 0; l < Terrain::patchLayers; ++l)
				UncacheTilesPrimitives(k, l);
		}
...
void TerrainRender::UpdateCache()
{
	...
	for(int j=j1; j<=j2; ++j)
	{
		if (g_terrain->IsPatchIndexInvalid(i,j))
			continue;
...
void TerrainRender::Render(const Terrain& terrain, const Vector2 &pos, int layer, float alpha)
{
	...
	if (DeferredRender::GetRenderPassIsShadow() || DeferredRender::GetRenderPassIsEmissive())
	{
		...
		// use the shadow color for background render pass
		if (DeferredRender::GetRenderPassIsDirectionalShadow())
			pd3dDevice->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG2);
	}
	
	pd3dDevice->SetTextureStageState( 1, D3DTSS_TEXCOORDINDEX, 0 );

	if (terrainCacheEnable && g_gameControlBase->IsGameplayMode())
	{
		for (int k = 0; k < tileBufferCount; ++k)
	...
	else
	{
		...
		const IntVector2 patchOffset = g_terrain->GetPatchOffset(pos);
		int i1 = patchOffset.x-Terrain::renderWindowSize;
		int i2 = patchOffset.x+Terrain::renderWindowSize;
		int j1 = patchOffset.y-Terrain::renderWindowSize;
		int j2 = patchOffset.y+Terrain::renderWindowSize;
		if (!g_gameControlBase->IsGameplayMode())
			g_editor.GetTerrainRenderWindow(patchOffset.x, patchOffset.y, i1, i2, j1, j2);

		// loop over all the patches and tiles in the window
		for(int i = i1; i <= i2; ++i)
		for(int j = j1; j <= j2; ++j)
		{
			if (terrain.IsPatchIndexInvalid(i,j))
				continue;
...
void TerrainRender::CacheTilesPrimitives(int tileBufferIndex, const TerrainPatch& patch, int layer)
{
	...
	{
		SurfaceEdgeTile(Vector2 _pos, BYTE surface, BYTE edge, BYTE _tileSet) : pos(_pos), surfaceData(surface), edgeData(edge), tileSet(_tileSet) {}

		static bool SortCompare(const SurfaceEdgeTile& first, const SurfaceEdgeTile& second) 
		{ 
			const GameSurfaceInfo& surfaceInfo1 = GameSurfaceInfo::Get(first.surfaceData);
			const GameSurfaceInfo& surfaceInfo2 = GameSurfaceInfo::Get(second.surfaceData);
			
			if (first.tileSet == second.tileSet && surfaceInfo1.IsSameRenderGroup(surfaceInfo2))
				return false;

			return (first.surfaceData < second.surfaceData && first.tileSet < second.tileSet);
		}
	...
	for(int yPatch=0; yPatch<Terrain::patchSize; ++yPatch)
	{
		...
		if (surface0 && tile.GetSurfaceHasArea(0))
			tileList.push_back(SurfaceEdgeTile(pos, surface0, tile.GetEdgeData(), tile.GetTileSet())); 
			
		const BYTE surface1 = tile.GetSurfaceData(1);
		if (surface1 && tile.GetSurfaceHasArea(1))
			tileList.push_back(SurfaceEdgeTile(pos, surface1, tile.GetInvertedEdgeData(), tile.GetTileSet())); 
	}
	...
	for (list<SurfaceEdgeTile>::iterator it = tileList.begin(); it != tileList.end(); ++it) 
	{
		...
		if (first || lastTileSet != tile.tileSet || !surfaceInfo.IsSameRenderGroup(lastSurfaceInfo))
		{
			// init the new group
			first = false;
			...
			tileRenderGroup.texture = Terrain::GetTileSetTexture(tile.tileSet);
		...
		// degerate tri on start
		SetupVert(&lockedVerts[vertexIndex++], tile.pos + edgeVerts[0], edgeVerts[0], surfaceInfo, surfaceData);
		SetupVert(&lockedVerts[vertexIndex++], tile.pos + edgeVerts[0], edgeVerts[0], surfaceInfo, surfaceData);
		...
		while (1)
		{
			...
			SetupVert(&lockedVerts[vertexIndex++], pos, edgeVert, surfaceInfo, surfaceData);
			if (rightPos > leftPos)
			{
...
void TerrainRender::RenderCached(int tileBufferIndex, int layer, float alpha)
{
	...
	for (int i = 0; i < CachedTileBuffer::maxGroups; ++i)
	{
		...
		if (Terrain::tileSetCount > 0)
			texture = g_render->GetTexture(tileRenderGroup.texture);
		else
		...
		bool deferredMapAlphaFix = false;
		if (DeferredRender::GetRenderPassIsDeferred())
		{
			if (!texture)
			{
				...
				if (Terrain::tileSetCount > 0)
					texture = g_render->GetTexture(tileRenderGroup.texture, false);
				else
					texture = g_render->GetTexture(surfaceInfo.ti, false);
				deferredMapAlphaFix = true;
			...
			else if (DeferredRender::GetRenderPassIsEmissive() || DeferredRender::GetRenderPassIsSpecular() || DeferredRender::GetRenderPassIsNormalMap())
			{
				...
				if (Terrain::tileSetCount > 0)
					textureDiffuse = g_render->GetTexture(tileRenderGroup.texture, false);
				else
...
void TerrainRender::RenderTiles(TerrainTile* tiles, const Vector2& pos, int width, int height, int layers, int firstLayer, float alpha)
{
	...
	for(int l=layers-1; l>=0; --l)
	{
		...
		const BYTE surface0Index = tile.GetSurfaceData(false);
		if (surface0Index != 0 && tile.GetSurfaceHasArea(false))
		{
			// draw side 1
			...
			c.a *= alpha;
			RenderTile(tileOffset, edgeIndex, surface0Index, tile.GetTileSet(), surfaceInfo, c);
		}
		const BYTE surface1Index = tile.GetSurfaceData(true);
		if (surface1Index != 0 && tile.GetSurfaceHasArea(true))
		{
			// draw side 2
			...
			Color c = actualLayer == 0? surfaceInfo.color : surfaceInfo.backgroundColor;
			c.a *= alpha;
			RenderTile(tileOffset, edgeIndex, surface1Index, tile.GetTileSet(), surfaceInfo, c);
		}
...
void TerrainRender::RenderSlow(const TerrainPatch& patch, int layer, float alpha)
{
	// do a camera test on the full patch
	if (!g_cameraBase->CameraTest(patch.GetAABB()))
		return;
	...
	for(int yPatch=0; yPatch<Terrain::patchSize; ++yPatch)
	{
		...
		if (showEdgePoints && !renderPass && !tile.IsClear() && !tile.IsFull())
		{
			// draw edge points for debugging
			const Vector2 posA = tile.GetPosA() + tileOffset;
			const Vector2 posB = tile.GetPosB() + tileOffset;
			posA.RenderDebug(Color::Red(0.5f), .1f, 0.2f);
			posB.RenderDebug(Color::Blue(0.5f), .1f, 0.2f);
		}
	}
...
void TerrainRender::RenderToTexture(const Terrain& terrain, RenderToTileCallback preCallback, RenderToTileCallback postCallback)
{
	...
	pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, D3DCOLOR_RGBA(0,0,0,255), 1.0f, 0);
	pd3dDevice->BeginScene();
	{
		// set display transforms
		D3DXMATRIX matrixProjection;
		float size = Terrain::fullSize * Terrain::patchSize * TerrainTile::GetSize();
		D3DXMatrixOrthoLH(&matrixProjection, size, size, -1000, 1000);
			...
			for(int yPatch=0; yPatch<Terrain::patchSize; ++yPatch)
			{
				...
				const Vector2 tileOffset = patch.GetTilePos(xPatch, yPatch);
				{
					...
					if (surface0Index != 0 && tile.GetSurfaceHasArea(false))
					{
						const GameSurfaceInfo& surfaceInfo = GameSurfaceInfo::Get(surface0Index);
						const BYTE edgeIndex = tile.GetEdgeData();
						RenderTile(tileOffset, edgeIndex, surface0Index, tile.GetTileSet(), surfaceInfo, surfaceInfo.color);
					}
					const BYTE surface1Index = tile.GetSurfaceData(true);
					if (surface1Index != 0 && tile.GetSurfaceHasArea(true))
					{
						const GameSurfaceInfo& surfaceInfo = GameSurfaceInfo::Get(surface1Index);
						const BYTE edgeIndex = tile.GetInvertedEdgeData();
						RenderTile(tileOffset, edgeIndex, surface1Index, tile.GetTileSet(), surfaceInfo, surfaceInfo.color);
					}
				}
			}
		}
...
void TerrainRender::InitDeviceObjects()
{
	...
	for (int i = 0; i < tileBufferCount*Terrain::patchLayers; ++i)
	{
		...
		for (int j = 0; j < CachedTileBuffer::maxGroups; ++j)
		{
			...
			trg.vertexCount = 0;
			trg.surfaceID = 0;
			trg.texture = GameTexture_Invalid;
		}
	}
...
inline void TerrainRender::RenderTile(const Vector2& position, const BYTE edgeIndex, const BYTE surfaceID, const BYTE tileSet, const GameSurfaceInfo& surfaceInfo, Color color)
{
	if (Terrain::tileSetCount > 0)
	{
		...
		const GameTextureID ti = Terrain::GetTileSetTexture(tileSet);
...
inline void TerrainRender::SetupVert(TERRAIN_VERTEX* vertex, const Vector2& worldPos, const Vector2& localPos, const GameSurfaceInfo& surfaceInfo, const BYTE surfaceID)
{
...
TerrainLayerRender::TerrainLayerRender(const Vector2& pos, int _layer, int _renderGroup) :
	...
	layer(_layer),
	desiredRenderGroup(_renderGroup),
	lightShadows(false),
...
void TerrainLayerRender::Update()
{
	if (g_gameControlBase->IsTileEditMode() && layer == g_editor.GetTileEditor().GetLayer())
	{
		// pop it to the top
		GameObject::SetRenderGroup(10000);
	}
	else
		SetRenderGroup(desiredRenderGroup);
}

void TerrainLayerRender::Render()
{
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR2, L"Terrain Render" ); // These events are to help PIX identify what the code is doing
	...
	{
		// change alpha for layer you are editing
		if (g_editor.GetTileEditor().GetLayer() != layer)
			alpha *= 0.5f;
	}
