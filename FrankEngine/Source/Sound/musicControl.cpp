...
float MusicControl::masterVolume = 0.8f;
...
void MusicControl::Transition(const WCHAR *filename, bool loop, float time)
{
	if (wcscmp(filename, openFilename) == 0)
		return;

	if (transitionTimer.IsValid() && wcscmp(filename, transitionFilename) == 0)
		return;
...
{
	bool pause = g_sound->GetMusicPlayer().IsPaused();
	swscanf_s(text.c_str(), L"%d", &pause);
	g_sound->GetMusicPlayer().Pause(pause);
}
...
void MusicControl::Play( bool _loop )
{
	...
	LONG lVolume = (LONG)(DSBVOLUME_MIN + masterVolume * volumeScale * transitionVolumeScale * (DSBVOLUME_MAX - DSBVOLUME_MIN - 1));
	if (lVolume == 0)
		lVolume = 1;
...
void MusicControl::Update()
{
	if (transitionTimer.IsValid())
	{
		float p = Percent(float(transitionTimer), transitionTime, 0.0f);
		//SetVolume(0.6f + 0.4f*p);
		transitionVolumeScale = sqrt(p);
