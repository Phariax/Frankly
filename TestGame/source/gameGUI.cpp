...
void GameGui::Init()
{
	...
	{	
		...
		mainDialog.AddStatic( ControlId_onScreenText, L"", 0, 0, 0, 0 );
		mainDialog.GetControl( ControlId_onScreenText )->SetTextColor(Color::BuildBytes(255, 255, 255, 0));
		mainDialog.GetControl( ControlId_onScreenText )->GetElement( 0 )->dwTextFormat = DT_TOP | DT_CENTER | DT_NOCLIP | DT_WORDBREAK;
		mainDialog.GetControl( ControlId_onScreenText )->SetVisible(true);
		
		mainDialog.AddButton( ControlId_button_play, L"Play", 0, 0, 0, 0 ); 
...
void GameGui::Refresh()
{
	...
	if (onScreenTextTime.IsValid() && onScreenTextTime < 5)
	{
		// show on screen text display
		const float percent = Percent((float)onScreenTextTime, 5.0f, 4.0f);
		mainDialog.GetStatic( ControlId_onScreenText )->SetText(onScreenTextString.c_str());
		mainDialog.GetStatic( ControlId_onScreenText )->SetTextColor(Color::BuildBytes(255, 255, 255, (int)(255.0f*percent) ));
	}
	else
	{
		mainDialog.GetStatic( ControlId_onScreenText )->SetTextColor(Color::BuildBytes(255, 255, 255, 0));
	}

	{
		...
		// hide on screen text when paused
		mainDialog.GetControl( ControlId_onScreenText )->SetVisible(!isPaused);
	}
}
...
void GameGui::OnResetDevice()
{
...

	{
		...
		y = g_backBufferHeight - 130;
		mainDialog.GetControl( ControlId_onScreenText )->SetLocation( x, y );
	}
	{
		// button bar
...
static void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		{	
			// this code triggers the user's web broswer to open the website
